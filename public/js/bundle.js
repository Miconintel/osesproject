...
var removeCart = function removeCart(e) {
  ...
  if (clicked) {
    console.log("allState ".concat(allState)); // remove item from cart
...
parentCartContainer.addEventListener('click', removeCart); /// SET HERO HEIGHT
//
//
//
...
var ChangeHeroSize = function ChangeHeroSize() {
  ...
  // get headerheight
  var headerHeight = parseInt(window.getComputedStyle(header).height); // getslidereigt

  var sliderHeight = parseInt(window.getComputedStyle(slider).height); // console.log(sliderHeight);
  // compute new height

  var herowidth = hero.getBoundingClientRect().width * 1;
  console.log(herowidth);
  if (herowidth > 390) slider.style.maxHeight = "".concat(sliderHeight - headerHeight, "px");
};
...
dotContainer.addEventListener('click', moveWithDots); // SMOOTH SCROLL WITH EVENT DELEGATION
//
//
...
navContainer.addEventListener('click', function (e) {
  if (e.target.classList.contains('nav--link')) {
    ...
    elementOfEquivalentId.scrollIntoView({
      behavior: 'smooth'
    });
  }
...
var scrollOnPage = function scrollOnPage() {
  var rectangle = {
    pageClicked: false
  };
  ...
  pageContainer.addEventListener('click', function (e) {
    var pageButton = e.target.closest('.page--link');

    if (pageButton) {
      rectangle.pageClicked = true;
      var b = pageContainer.getBoundingClientRect();
      rectangle.top = b.top;
      rectangle.left = b.left;
      window.localStorage.setItem('rectangle', JSON.stringify(rectangle));
    }
  });
  var useRect = JSON.parse(window.localStorage.getItem('rectangle'));
